...
{		
	...
	private	editable			var	speedLevelCapDefault	: int;							default	speedLevelCapDefault	= 2;//rs: Default is too high 3;
	...
	private	editable 			var	minSpeedTotal			: float;						default	minSpeedTotal			= 6.0f;
	...
	// Stop	
	private	editable 			var	speedToBrake			: float;						default	speedToBrake			= 2.5f;//rs:Default too high 3.0f;
	private	editable 			var	speedToStop				: float;						default	speedToStop				= 1.5f;//rs:Default too high 0.5f;
	...
	{
		//skating restored: enable attacking again
		// TEMP HACK
		return false;
		...
		if( m_ExplorationO.m_InputO.IsSkateAttackJustPressed() )
		{
			...
			// skating restored: make this do light attack
			//LogChannel('COMBATTARGET',GetCiriPlayer().GetCombatActionTarget(EBAT_LightAttack));
			//GetCiriPlayer().PushCombatActionOnBuffer(EBAT_LightAttack,BS_Pressed);
			//GetCiriPlayer().ProcessCombatActionBuffer();
			return true;
		}
	...
	public function ImpulseToNextSpeedLevel( baseImpulse : float )
	{
		...
		//rs: Hack combat log to add impulse boost amount
		theGame.witcherLog.AddMessage("Added Impulse:"+FormatF(aux));
		//rs
	...
	{
		// rs: Correct Logic. speedLevelTotal is not the highest speed level!
		if( speedLevelCur < speedLevelTotal - 1 )
		//rs
		{	
	...
	//rs: Add this for combat log use for speed levels
