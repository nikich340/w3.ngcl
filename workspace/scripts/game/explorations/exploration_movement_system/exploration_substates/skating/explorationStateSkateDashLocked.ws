...
{		
	...
	private editable	var	targetMaxAngle			: float;		default	targetMaxAngle		= 330.0f;//rs:change angle range 25.0f;
	private editable	var	targetMaxAngleFlow		: float;		default	targetMaxAngleFlow	= 330.0f;//rs:change angle range 45.0f;
	private editable	var	reachSideDistance		: float;		default	reachSideDistance	= 2.0f;
	private editable	var	targetSideDistance		: float;		default	targetSideDistance	= 0.25f;//rs:change side distance 1.0f;
	private	editable	var inputAngleInfluence		: float;		default	inputAngleInfluence	= 15.0f;
	
	// Movement
	private				var	speed					: float;
	private				var	speedMinMax				: float;		default	speedMinMax			= 6.0f;//rs:change side distance 8.0f;
	private				var	impulseMax				: float;		default	impulseMax			= 0.5;//rs:change side distance 5.5f;
	private				var	impulseMaxFlow			: float;		default	impulseMaxFlow		= 1.5;//rs:change side distance 6.5f;
	...
	protected editable	var	timeScaleSpeed			: float;		default	timeScaleSpeed		= 0.50f;
	...
	private editable	var	afterAttackImpulse	: float;	default	afterAttackImpulse			= 0.5f;//rs:greatly reduce impulse 5.0f;
	private editable	var	isEnabled			: bool;		default isEnabled					= true;
	...
	{				
		...
		//rs: leave this be. It doesn't work properly
		//UpdateAttackFlow();
	...
	private function UpdateAttack( _Dt : float )
	{
		...
		//rs: Rework this for better reliability
	...
	private function UpdateAttackFlow()
	{
		...
		if( !attacked )
		{
			...
			LogChannel('UPDATEDASHATTACK',attacking);
			// Pwrfect time
			if( toTargetDistance < attackDistGapPerfect )
			{
				if( attacking )
				{
					...
					//rs: Add messages for testing
					theGame.witcherLog.AddMessage("Perfect Timing!");
				}
			}
			// Acceptable time
			else if( toTargetDistance < attackDistGap )
			{
				if( attacking )
				{
					...
					theGame.GetGameCamera().PlayAnimation( animation );
					//rs: Add messages for testing
					theGame.witcherLog.AddMessage("Acceptable Timing!");
				}
			}
	...
	private function OrientToTarget()
	{
		...
		movAdj.AdjustmentDuration( adjustorTicket, 0.1f );
		...
		//rs: restore this
		movAdj.LockMovementInDirection( adjustorTicket, direction );
	}
	...
	private function GetAValidTarget() : bool
	{
		...
		//rs: print targets aquired to log
		LogChannel('TARGETS',targets.Size());
		...
		*/
		...
		for( i = 0; i < results; i+= 1 )
		{
			...
			if( target )
			{
				...
				//rs: Log if target acceptable
				LogChannel('TARGET','ACCEPTABLE');
				// Acceptable target
				return true;
			}
		}
