...
exec function minimap_run(z : float) {
	...
	if (!cam) {
		...
		NGCL_Notify_Shared("CAMERA CREATED!");
	} else {
		NGCL_Notify_Shared("CAMERA FOUND!");
...
exec function bcry() {
	thePlayer.PlayBattleCry( 'BattleCryTaunt', 1.f, true, true );
}
...
exec function NGCL_TestSkating(enable : bool)
{
	...
	NGCL_Notify_Shared("NGCL_TestSkating = " + enable);
}
